...
into #LinePressSolo
FROM [ODSSQL].[CygNet].[dbo].[TEMPERATURE] with (nolock)
group by Facility 


Select a.Facility,a.RecordTime,b.Value into #Combo from #LinePressSolo a
INNER join [ODSSQL].[CygNet].[dbo].[TEMPERATURE] b
...
INTO #Spatial
from #LPSites C
INNER JOIN [ArcGis_Master].[dbo].[CYGNET_NAMES] AGS
ON C.Facility = AGS.CYGNET_NAME


TRUNCATE TABLE [ArcGis_Master].[dbo].[CYGNET_TEMPERATURE]
INSERT INTO [ArcGis_Master].[dbo].[CYGNET_TEMPERATURE] (OBJECTID, SHAPE,Facility,RecordTime,Value,Well_Short_Name,Type)
select ROW_NUMBER() OVER(ORDER BY Facility  ASC) as OBJECTID
,Shape as SHAPE
,Facility
,RecordTime
