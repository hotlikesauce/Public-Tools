...
into #LinePressSolo
FROM [ODSSQL].[CygNet].[dbo].[CASING_PRES_ROLLING_DAY] with (nolock)
group by Facility 


Select a.Facility,a.RecordTime,b.Value into #Combo from #LinePressSolo a
left outer join [ODSSQL].[CygNet].[dbo].[CASING_PRES_ROLLING_DAY] b
...
TRUNCATE TABLE [ArcGis_Master].[dbo].[CYGNET_CASING_PRESSURE]
INSERT INTO [ArcGis_Master].[dbo].[CYGNET_CASING_PRESSURE] (OBJECTID, SHAPE,Facility,RecordTime,Value,Well_Short_Name,Type)
select ROW_NUMBER() OVER(ORDER BY Facility  ASC) as OBJECTID
,Shape as SHAPE
,Facility
,RecordTime
